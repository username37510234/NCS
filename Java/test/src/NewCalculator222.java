...
// ***** 클래스명 변경
class NewCalculator222  implements ActionListener
{	
...
//	JMenu menu3;  //***** 메뉴제거
	...
	public NewCalculator222(){
		//frame=new JFrame("계산기"); // ***** 타이틀 '계산기 예제'로 변경
		frame=new JFrame("계산기예제");
...
//		menuBar.add(menu3);	
	...
	public void actionPerformed(ActionEvent e){
		...
		try{	
			//연산기호가 아닌 숫자이면 true 이다.***** 숫자 판단에 C를 %로 변경
			...
			if(str=="="){		
				...
				for(int a=0,h=1; a<ee.size(); a++,h++){   // ArrayList 크기만큼 반복
					...
					if(ve>0){							// 연산자 누적 모두 사용할때까지 반복.
						...
						else if(store.get(a)=="%"){		// ***** 계산로직 % 추가 : 계산이 되어야한다							
							try{				
								sum=sum%ee.get(h);
	...
	{
		// ***** 클래스명 변경
		NewCalculator222 cal=new NewCalculator222();
		cal.gui();
	}// main()
}// class
